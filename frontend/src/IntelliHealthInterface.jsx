  ...
  const generatePDFReport = async () => {
    try {
      ...
      try {
        ...
        if (response.data.success) historyData = response.data.history || [];
      } catch {
        historyData = buildSessionHistory();
      }
      const doc = new jsPDF();
      const pageWidth = doc.internal.pageSize.getWidth();
      const pageHeight = doc.internal.pageSize.getHeight();
      ...
      if (historyData.length === 0) {
        ...
        doc.text('No consultation history available.', 20, yPos); yPos += 15;
      } else {
        historyData.forEach((c, i) => {
          if (yPos > 240) { doc.addPage(); yPos = 20; }
          doc.setFontSize(10); doc.setFont('helvetica', 'bold'); doc.setTextColor(88, 28, 135);
          doc.text(`CONSULTATION ${i + 1}: ${(c.query_type || 'Consultation').toUpperCase()}`, 20, yPos);
          yPos += 5; doc.setFont('helvetica', 'normal'); doc.setTextColor(0, 0, 0);
          const q = doc.splitTextToSize(c.query || 'No query', pageWidth - 40);
          doc.text(q, 20, yPos); yPos += q.length * 5 + 3;
          const r = doc.splitTextToSize(c.response || 'No response', pageWidth - 40);
          doc.text(r, 20, yPos); yPos += r.length * 5 + 8;
        });
      }
