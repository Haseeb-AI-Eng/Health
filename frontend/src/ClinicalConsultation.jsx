...
const ClinicalConsultation = ({ patientData, onBack, onLogout }) => {
 ...
 const [conversationType, setConversationType] = useState('generic'); // 'generic', 'clinical', 'lifestyle', 'preventive', 'lab-results'
 ...
 // For general conversation mode, don't require disease/medication
 ...
 custom_query: query,
 query: query || "Provide clinical insights for this case",
 ...
 { role: 'assistant', content: response.data.ai_response.response }
 ]);
 }
 } catch (error) {
 ...
 >
 {(() => {
 const config = getDecisionConfig(analysisResult.final_decision);
 const DecisionIcon = config.icon;
 return (
 <>
 <div className={`${config.bgGradient} px-6 py-5 border-b ${config.borderColor}`}>
 <div className="flex items-center justify-between">
 <div className="flex items-center gap-3">
 <div className={`w-12 h-12 rounded-xl ${config.bgColor} flex items-center justify-center`}>
 <DecisionIcon className={`text-2xl ${config.color}`} />
 </div>
 <div>
 <h2 className={`text-xl font-bold ${config.color}`}>
 {analysisResult.final_decision}
 </h2>
 <p className="text-sm text-gray-600">Risk Level: {analysisResult.risk_level}</p>
 </div>
 </div>
 <div className="text-right">
 <div className="text-3xl font-bold text-gray-900">
 {analysisResult.risk_score?.score || 0}/100
 </div>
 ...
 <div className="p-6">
 <h3 className="text-sm font-semibold text-gray-700 mb-2">Clinical Explanation</h3>
